...
#define TX_BUFFER_FULL          (IsBufferFull(&g_ui32UARTTxReadIndex,  \
                                              &g_ui32UARTTxWriteIndex, \
                                              UART_TX_BUFFER_SIZE))
#define ADVANCE_TX_mensagem_a_distribuirDEX(Index) \
...
#define RX_BUFFER_FULL          (IsBufferFull(&g_ui32UARTRxReadIndex,  \
                                              &g_ui32UARTRxWriteIndex, \
                                              UART_RX_BUFFER_SIZE))
#define ADVANCE_RX_mensagem_a_distribuirDEX(Index) \
...
UARTPrimeTransmit(uint32_t ui32Base)
{
    ...
    if(!TX_BUFFER_EMPTY)
    {
        ...
        while(MAP_UARTSpaceAvail(ui32Base) && !TX_BUFFER_EMPTY)
        {
            ...
            ADVANCE_TX_mensagem_a_distribuirDEX(g_ui32UARTTxReadIndex);
        }
    ...
    for(uIdx = 0; uIdx < ui32Len; uIdx++)
    {
        ...
        if(pcBuf[uIdx] == '\n')
        {
            if(!TX_BUFFER_FULL)
            {
                ...
                ADVANCE_TX_mensagem_a_distribuirDEX(g_ui32UARTTxWriteIndex);
            }
            else
            {
                //
...
UARTgets(char *pcBuf, uint32_t ui32Len)
{
    ...
    while(1)
    {
        ...
        if(!RX_BUFFER_EMPTY)
        {
            ...
            ADVANCE_RX_mensagem_a_distribuirDEX(g_ui32UARTRxReadIndex);
...
UARTPeek(unsigned char ucChar)
{
    ...
    for(iCount = 0; iCount < iAvail; iCount++)
    {
        ...
        else
        {
            ...
            //
            ADVANCE_RX_mensagem_a_distribuirDEX(ui32ReadIndex);
        }
    }
    ...
    if(ui32Ints & (UART_INT_RX | UART_INT_RT))
    {
        ...
        while(MAP_UARTCharsAvail(g_ui32Base))
        {
            ...
            if(!RX_BUFFER_FULL)
            {
                ...
                ADVANCE_RX_mensagem_a_distribuirDEX(g_ui32UARTRxWriteIndex);
